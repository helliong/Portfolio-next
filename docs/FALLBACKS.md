...
| [`src/components/SelfServicePopup.tsx:247`](file:///d:/work/Portfolio-next/src/components/SelfServicePopup.tsx#L247) | Сеть / Формы | Fallback задержки: `60` секунд | Значение `retryAfter` от сервера Upstash Redis | Сервер вернул 429 без валидного числа секунд `retryAfter` |
