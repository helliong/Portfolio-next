...
// Keep the limiter outside the request handler so warm Vercel instances can reuse it.
const ratelimit =
  redisUrl && redisToken
    ? new Ratelimit({
        redis: new Redis({ url: redisUrl, token: redisToken }),
        limiter: Ratelimit.slidingWindow(3, "1 h"),
        prefix: `portfolio:${deploymentEnvironment}:contact`,
        timeout: 1_500,
      })
    : null;
...
export async function POST(request: Request) {
  try {
    ...
    if (!ratelimit && isVercelDeployment) {
    ...
    if (ratelimit) {
      const result = await ratelimit.limit(getRateLimitIdentifier(ip ?? "local-development"));
